...
describe('POST /auth/register', () => {
  const testUserEmails = [
    ...
    'test@example.com',
    'john.token@example.com',
    'john.org@example.com',
    'jane.org@example.com'
  ];
  ...
  it('Should ensure token expires at the correct time and correct user details are found in the token', async () => {
    const res = await request(app)
      .post('/auth/login')
      .send({
        email: 'john@example.com',
    ...
    expect(decoded.userId).toBe(res.body.data.user.userId);
    expect(decoded.email).toBe(res.body.data.user.email);
    expect(decoded.firstName).toBe(res.body.data.user.firstName);
    expect(decoded.lastName).toBe(res.body.data.user.lastName);
    ...
    expect(expirationTime).toBeLessThan(currentTime + oneHour);
  });
